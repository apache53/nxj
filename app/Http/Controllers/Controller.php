...
class Controller extends BaseController
{
    ...
    //判断平台游戏关联
    protected function checkPlatformGameRelation($platform_id, $game_id)
    {
        $res = H5YunPlatformGame::getPlatGameRelation($platform_id, $game_id);
        if (!empty($res)) {

            if ($res['operation_state'] != Config::get('constants.PLATFORM_GAME_ONLINE_STATUS')) {
                return array(
                    "error" => 4001,
                    "msg" => "platform game not online",
                    "res" => $res
                );
            }
        ...
        return array(
            "error" => 4002,
            "msg" => "platform game not related",
            "res" => array()
        );
    }

    //判断游戏参数
    protected function checkGame($game_id)
    {
        $game = H5YunGame::getListMap($game_id);
        if (isset($game[$game_id]) && !empty($game[$game_id])) {

            $game_info = $game[$game_id];
            if ($game_info['operation_state'] != Config::get('constants.GAME_ONLINE_STATUS')) {
                return array(
                    "error" => 4001,
                    "msg" => "game not online",
                    "res" => $game_info
    ...
    //判断平台参数
    protected function checkPlatform($paltform_id)
    {
        $platform = H5YunPlatform::getListMap($paltform_id);
        if (isset($platform[$paltform_id]) && !empty($platform[$paltform_id])) {

            $platform_info = $platform[$paltform_id];
            if ($platform_info['operation_state'] != Config('constants.PLATFORM_ONLINE_STATUS')) {
                return array(
                    "error" => 4001,
                    "msg" => "platform not online",
                    "res" => $platform_info
                );
            }
