...
class VerifyCode{
    ...
    public function setFont($font) {
        if (is_array($font)){
            ...
            if (isset($font['left']) && (!is_numeric($font['left']) || $font['left']<0 ||
            ...
            if (isset($font['top']) && (!is_integer($font['top']) || $font['top']<0 ||
                $font['top']>$this->image['height'])) {
                $font['top'] = ($this->image['height'] - $font['size']) / 2
                    + $font['size'];
            }else{
                $font['top'] = ($this->image['height'] - $this->font['size']) / 2
                    + $this->font['size'];
            }
            ...
            $this->font = array('space'=>5, 'size'=>12,
                ...
                'file'=>dirname(__FILE__).'/arial.ttf');
    ...
    public function paint($filename='')
    {
        ...
        // 生成的验证码
        $the_code = '';
//print_r($this->font);exit;
        // 向图像中写入字符
        $num = count($code);
        $current_left = isset($this->font['left'])?$this->font['left']:0;
        $current_top  = isset($this->font['top'])?$this->font['top']:0;
        $this->font['file'] = isset($this->font['file'])?$this->font['file']:dirname(__FILE__).'/arial.ttf';
        for ($i=0; $i<$num; $i++)  {
